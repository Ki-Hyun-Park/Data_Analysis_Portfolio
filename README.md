...
- Summary
     ...
     <p>(2). 데이터 전처리 <br/>
     - PCR을 위한 반응 변수 값 변환(0 = 방문팀 승리 / 1 = 홈팀 승리) <br/>
     ...
     - Lasso, LDA, Logistic, PCR, Ridge를 통한 모델링 및 결과 예측(Ridge가 가장 낮은 RMSE 및 가장 높은 유사도 보유) </p>
     ...
     - PCR 및 Ridge는 다중공선성 문제점을 해결할 수 있고 이점은 더 높은 유사도의 예측 결과물을 만들 수 있다. </p>
...
- Summary
     ...
     <p>(2). 데이터 전처리 <br/>
     - EDA 및 비교분석을 위한 특정 변수 제작: Hours(졸업까지 소요되는 총 학점), Out class(선행수업 중 상급과목에 가장 많이 적용되는 갯수), In class(상급과목 중 가장 많은 선행수업을 요구하는 갯수),
       Longest path(학기마다 연속적으로 들어야하는 수업 횟수), Bottlenecks(필요 선행수업이 3개 이상 요구되는 수업 갯수), Avg TTD(평균적으로 졸업까지 소요되는 학기 갯수),
       Stem(Yes = STEM 전공, No = Non-STEM 전공) </p>
...
- Summary
     ...
     <p>(2). 데이터 전처리 <br/>
     - 혈압 수치: (운동량/음식물을 받기전 혈압 수치 - 받은후 혈압 수치) <br/>
     - 운동량: 달리지않고 20분동안 휴식(Low Level), 5km 달리기(High Level) <br/>
     - 식품 섭취: 올리브 오일 500mg을 함유한 타블렛 복용(Low Level), 코카잎 500mg 복용(High Level) <br/>
     ...
     <p>(6). 인사이트 <br/>
     - "식품 섭취" 그리고 "운동량" 모두 개별적으로는 혈압 수치에 영향을 미치지 못했지만 두가지의 상호작용 효과는 혈압 수치에 영향을 미치는 것으로 나타났다. <br/>
     - 블록(나이) 그리고 블록(성별) 모두 혈압 수치에 영향을 미치지 못했다. </p>
     
     <p>(7). 피드백 <br/>    
     - The Islands에서 데이터를 수집하는데 소요되는 로딩 시간과 프로젝트를 마무리 하는데까지 주어진 시간을 고려했을때 48명의 데이터밖에 수집할수 없었고 48명의 데이터는 너무 작았기에 편향이 있었을것이라
       생각한다. </p>
